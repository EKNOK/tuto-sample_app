...
そして、画面にて、正しいメールアドレスを送信した時には、flash[:seccess]のメッセージがルートURLで表示。  
...
## 12.3 パスワードを設定する。
12.11で無事、送信メールを生成できたので、  
次は、PassswordResetsコントローラの`edit`アクションの実装を進めていく。  
また、11.3.3の時と同様に、統合テストを使ってうまく動作しているかテストも行っていく。  
.  
.  
