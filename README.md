...
.  
.  
JS-ERbファイルの内部はDOM(Document Object Model)を使ってページを操作するため、RailsがjQueryを自動的に提供している。これによりjQueryライブラリの膨大なDOM操作用メソッドが使えるようになるが、今回使うのはわずか2つ。まずはドル記号($)とCSS idを使って、DOM要素にアクセスする文法二ついて知る必要がある。例えば、`follow_form`の要素をjQuery
で操作するには、  
