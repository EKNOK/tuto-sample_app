...
.  
.  
(1)については、`edit`と`update`アクションに次のようなメソッドとbeforeフィルターを用紙することで対応できる。  
```
# (1)の対応案
...
例えば、(2)二ついては、更新が失敗したときに`edit`のビューが再描画され、12.14のパーシャルにエラーメッセージ表示されるようにすれば、解決できる。  
